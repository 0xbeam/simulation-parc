...
import { renderMarkdown as _renderMarkdown } from '../composables/useMarkdown'
...
const renderMarkdown = (content) => {
  return _renderMarkdown(content, { stripLeadingH2: true })
}
...
let pollCount = 0
const MAX_POLL_COUNT = 300 // 300 * 2s = 10 minutes

const fetchAgentLog = async () => {
  pollCount++
  if (pollCount > MAX_POLL_COUNT && !isComplete.value) {
    isComplete.value = true
    currentSectionIndex.value = null
    emit('update-status', 'timeout')
    stopPolling()
    return
  }
